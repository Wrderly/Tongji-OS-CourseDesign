...
uint64
vmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int need_free)
{
  if(newsz >= oldsz)
    return oldsz;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, need_free);
  }
...
int
ukvmcopy(pagetable_t old, pagetable_t new, uint64 start, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
